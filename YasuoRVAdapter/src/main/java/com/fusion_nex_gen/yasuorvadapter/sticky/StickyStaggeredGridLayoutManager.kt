...
import com.fusion_nex_gen.yasuorvadapter.sticky.StickyCallBack.ViewSetup
import java.util.*
import kotlin.math.max
import kotlin.math.min

class StickyStaggeredGridLayoutManager<T> : StaggeredGridLayoutManager where T : RecyclerView.Adapter<*>, T : StickyCallBack {
        ...
        if (headerCount > 0 && childCount > 0) {
            ...
            for (i in 0 until childCount) {
                ...
                if (isViewValidAnchor(child, params)) {
                    anchorView = child
                    anchorIndex = i
                    anchorPos = params.bindingAdapterPosition
                    break
                }
            }
