...
//存储一个类型的各种属性
data class YasuoItemDataBindingConfig<T : Any, VH : YasuoDataBindingVH<ViewDataBinding>, VB : ViewDataBinding>(
    //item的布局id
    ...
    val variableId: Int = BR.item,
    //该类型Holder创建时的监听
    val createListener: (VB.(holder: YasuoDataBindingVH<ViewDataBinding>) -> Unit)? = null,
    //该类型的Holder绑定时的监听
    val bindListener: (VB.(holder: YasuoDataBindingVH<ViewDataBinding>) -> Unit)? = null,
    ////该类型的Holder绑定时的监听，带payloads
    val bindAndPayloadsListener: (VB.(holder: YasuoDataBindingVH<ViewDataBinding>, payloads: List<Any>?) -> Unit)? = null
...
data class YasuoItemNormalConfig<T : Any, VH : YasuoNormalVH>(
    ...
    //该类型的Holder绑定时的监听
    val bindListener: (T.(holder: YasuoNormalVH) -> Unit)? = null,
    ////该类型的Holder绑定时的监听，带payloads
    val bindAndPayloadsListener: (T.(holder: YasuoNormalVH, payloads: List<Any>?) -> Unit)? = null
) : YasuoBaseItemConfig<T, VH>(itemLayoutId, sticky, isFold, gridSpan, staggeredGridFullSpan)
...
data class YasuoItemViewBindingConfig<T : Any, VH : YasuoViewBindingVH, VB : ViewBinding>(
    ...
    //loadMore，空布局会默认为占满
    override var staggeredGridFullSpan: Boolean = false,
    var createBindingFun: (view: View) -> ViewBinding,
    //该类型Holder创建时的监听
